...
//go:generate sh -c "jq '( .resources[] | select(.name == \"vm\" or .name == \"postgres\") | .schema.attributes[] | select(.name == \"storage_size\") ).int64.computed_optional_required = \"optional\" | ( .resources[] | select(.name == \"postgres\") | .schema.attributes[] | select(.name == \"size\") ).string.computed_optional_required = \"computed_optional\" | ( .resources[] | select(.name == \"postgres\") | .schema.attributes[] | select(.name == \"parent\") ).string.computed_optional_required = \"computed_optional\" | ( .resources[] | select(.name == \"postgres\") | .schema.attributes[] | select(.name == \"private_subnet_name\") ).string.computed_optional_required = \"optional\" | ( .resources[] | select(.name == \"postgres\") | .schema.attributes[] | select(.name == \"restrict_by_default\") ).bool.computed_optional_required = \"optional\"' config/generated/provider_code_spec_mod.tmp2.json > config/generated/provider_code_spec_mod.tmp3.json"
