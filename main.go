...
//go:generate go run ./tools/preprocess_openapi config/ubicloud_openapi.yml config/generated/preprocessed_openapi.json
//go:generate go run github.com/hashicorp/terraform-plugin-codegen-openapi/cmd/tfplugingen-openapi generate --config config/tf_generator_config.yml --output config/generated/provider_code_spec.json config/generated/preprocessed_openapi.json
...
//go:generate sh -c "jq '( .resources[] | select(.name == \"vm\") | .schema.attributes[] | select(.name == \"storage_size\") ).int64.computed_optional_required = \"optional\"' config/generated/provider_code_spec_mod.tmp2.json > config/generated/provider_code_spec_mod.tmp3.json"
...
//go:generate sh -c "jq -f config/classify.jq config/generated/provider_code_spec_mod.tmp4.json > config/generated/provider_code_spec_mod.tmp6.json"
//go:generate sh -c "jq -f config/inject_restore_target.jq config/generated/provider_code_spec_mod.tmp6.json > config/generated/provider_code_spec_mod.tmp7.json"
//go:generate sh -c "jq -f config/plan_modifiers.jq config/generated/provider_code_spec_mod.tmp7.json > config/generated/provider_code_spec_mod.tmp8.json"
...
//go:generate sh -c "jq -e -f config/assert_postgres_resource.jq config/generated/provider_code_spec_mod.json > /dev/null"
