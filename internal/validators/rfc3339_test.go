...
// RFC3339() must accept exactly the values time.Parse(time.RFC3339) accepts after a trim,
// so plan-time validation and the apply-time restore body builder agree. The malformed
// cases (month 13, hour 24, second 60) are the regex/parser gap a coarse shape regex left
// open; null/unknown are skipped (an unknown computed target is parsed at apply).
