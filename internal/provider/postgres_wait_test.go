...
// newTestPostgresResource builds a postgresResource whose client talks to srv. It drives
// the real generated client against an httptest backend (a network-boundary fake), not a
// stub of the client, so the wait helpers exercise their actual request/response path.
...
// detailsHandler serves the detailed GET, returning the state produced by next() each call
// (and a 404 when next returns the empty string, to model a deleted database).
...
func TestWaitForPostgresRunningPollsUntilRunning(t *testing.T) {
	...
	srv := httptest.NewServer(detailsHandler(func() string {
		// creating, creating, then running: forces the helper to actually poll.
...
func TestWaitForPostgresRunningHonorsContextCancel(t *testing.T) {
	...
	// Parent cancellation must be classified as a cancellation, not a resource timeout (raising
	// timeouts.create cannot help); asserting the wording keeps the classification non-vacuous.
...
// A single detail GET that hangs must not outlive the configured timeout. The wait is bounded
// by a context derived from the timeout, so a stuck read surfaces as a Timeout within the
// budget instead of blocking on the original context until the provider-wide deadline (which
// could be hours away, or never). Without the derived context the helper hangs here and the
// 3s watchdog fires (Codex HIGH/MEDIUM).
func TestWaitForPostgresRunningBoundsStuckGet(t *testing.T) {
	...
	// Order matters: srv.Close() (registered first) runs last, after close(release) unblocks
	// the in-flight handler, so Close does not deadlock waiting on a blocked handler.
	defer srv.Close()
	defer close(release)
...
// Read must drop the resource from state on a confirmed 404 instead of erroring. A delete whose
// wait timed out preserves state; once the backend finishes the teardown the next refresh hits a
// 404, and erroring there would wedge a phantom resource in state (gone server-side, stuck in
// Terraform) until a manual `state rm`. RemoveResource on 404 is the idiomatic drift handling and
// lets the next plan converge cleanly (recreate, or nothing to destroy).
