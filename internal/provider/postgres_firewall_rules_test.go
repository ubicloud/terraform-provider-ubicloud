...
// TestGetPostgresFirewallRulesState guards the regenerated PostgresFirewallRule value
// mapping (issue 002c, decision D.1). The regenerated FirewallRulesValue carries four
// attributes (cidr, description, id, port); NewFirewallRulesValueMust panics when handed
// a partial attribute map, which is exactly what the create->read path hits when the
// backend seeds default firewall rules. A nil/empty input must yield a known empty list.
