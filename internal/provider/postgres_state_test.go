...
// The detailed GET a primary returns at state=creating: nil hostname, a host-less
// connection string, no resize/upgrade targets, and no restore times yet.
...
func TestSetPostgresStateResourceWiresReadSurface(t *testing.T) {
	...
	// size and storage_size are inputs that must round-trip from VmSize/StorageSizeGib
	// the way an import repopulates them; an unmapped one is left null after import.
	...
	// A null window must map to Int64Null, not 0: 0 is a real midnight window.
	...
	// hostname is null (not empty) at creating: the VM has no IP yet, so the
	// serializer emits a nil pointer (verified live).
	...
	// The shared tags helper emits datasource_postgres.TagsValue for both surfaces;
	// the object shape is structurally identical, so the resource schema accepts it.
...
// A wire null must map to Int64Null, not 0 (0 is a real midnight window). Built by
// JSON-unmarshal: a struct literal cannot express the API's null on this field.
