...
func (d *firewallRuleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	firewallRuleResp, err := d.uc.client.GetLocationFirewallFirewallRuleDetailsWithResponse(ctx, state.ProjectId.ValueString(), state.Location.ValueString(), state.FirewallName.ValueString(), state.Id.ValueString())
	if err != nil {
		resp.Diagnostics.AddError(
...
func GetFirewallRulesState(ctx context.Context, firewallRules []ubicloud_client.FirewallRule) (basetypes.ListValue, diag.Diagnostics) {
	...
	firewallRulesValues := make([]datasource_vm.FirewallRulesValue, 0, len(firewallRules))
