...
func (d *firewallRuleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	firewallRuleResp, err := d.uc.client.GetFirewallRuleDetailsWithResponse(ctx, state.ProjectId.ValueString(), state.Location.ValueString(), state.FirewallId.ValueString(), state.Id.ValueString())
	if err != nil {
		resp.Diagnostics.AddError(
